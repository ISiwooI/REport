...
public class CardManager : MonoBehaviour{
    ...
    public int normalCardCount{
        get{ return normalCards.Count; }
    }

    public int departmentCardCount{
        get{ return departmentCards.Count; }
    }
    ...
    private void Awake(){
        ...
        cardOP = new ObjectPool<CardObject>(
            ...
            , (co) => { co.gameObject.SetActive(true); }
            , (co) => { co.gameObject.SetActive(false); }
            , (co) => { Destroy(co.gameObject); }
            , true, 20, 40);
    }
    ...
    public void AlignCard(){
        ...
        foreach (CardObject co in departmentCards){
            ...
            co.Orderindex = i;
            Vector3 pos = new Vector3(dcr.position.x + ((dcl.position.x - dcr.position.x) / dcount * i),
                dcr.position.y + ((dcl.position.y - dcr.position.y) * i), 0);
        ...
        foreach (CardObject co in normalCards){
            co.sortingGroup.sortingOrder = i;
            co.Orderindex = i;
            Vector3 pos = new Vector3(ncr.position.x + ((ncl.position.x - ncr.position.x) / ncount * i),
                ncr.position.y + ((ncl.position.y - ncr.position.y) * i), 0);
            co.transform.DOMove(pos, 0.5f, false).SetEase(Ease.OutSine);
            i++;
        }
    }
